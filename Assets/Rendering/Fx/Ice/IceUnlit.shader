...
Shader "Fx/Ice/IceUnlit"
{
	Properties
	{
		_DiffuseTex ("DiffuseTex (RGBA)", 2D) = "white" {}
		_NormalTex("NormalTex (XYZ)", 2D) = "white" {}
		_IceColor("IceColor", Color) = (0.69,0.92,1,1)
		_ApplyReflections("ApplyReflections (Bool)", Float) = 1.0
	}
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 RenderWater(v2f i)
			{
				...
				if (_ApplyReflections == 1.0)
				{
					fixed3 ftcolor = tex2D(_BackgroundTex, uvscreen - normal.xz*0.1).rgb;
					//color = lerp(ftcolor, _IceColor, diffuse.a);
					color = lerp(color, ftcolor, clamp(Fresnel, 0.0, 1.0));
				}
